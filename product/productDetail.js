...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (productId) {
    fetch("../db.json")
      ...
      .then((data) => {
        product = data.oliveyoungProduct.find(
        ...
        if (product) {
          ...
          const review = document.querySelector(".tab:nth-child(2) label");

          const reviewNum = Number(product.review.replace(",", ""));

          const reviewStr = formatter.format(reviewNum);

          if (reviewNum < 1000) {
            review.innerHTML = `리뷰 ${reviewStr}`;
          } else {
            review.innerHTML = `리뷰 +999`;
          }

          console.log(reviewNum, reviewStr);
